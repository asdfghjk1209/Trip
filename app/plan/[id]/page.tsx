...
import { 
  ...
  LogOut, Moon, Sun, Heart, MessageCircle, CheckSquare, Briefcase, Shirt, Sparkles, User,
...
const TYPE_CONFIG: Record<string, { 
  ...
  transport: { 
      color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-200', ring: 'ring-indigo-100',
      icon: TrainFront, label: '交通',
      barColor: 'bg-indigo-500', badgeBg: 'bg-indigo-50', badgeText: 'text-indigo-600'
  },
  rest: { 
    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            if (value.length > 1 && showDropdown) {
                setLoading(true);
                try {
                    const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(value)}&limit=10&accept-language=zh-CN&countrycodes=cn&addressdetails=1`);
        ...
        <div className="relative" ref={wrapperRef}>
             <div className="relative">
                ...
                <input 
                    ...
                    className="w-full p-3 pl-9 bg-zinc-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-zinc-200 text-sm"
                />
            ...
            {showDropdown && suggestions.length > 0 && (
                <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-xl border border-zinc-100 max-h-60 overflow-y-auto animate-in fade-in zoom-in-95">
                    {suggestions.map((item) => {
                        ...
                        const subName = item.address.city || item.address.province || item.address.district || "";
                        return (
                            <div key={item.place_id} onClick={() => {
                                ...
                                <div className="font-medium text-sm text-zinc-800 flex justify-between">
                                    <span>{mainName}</span>
                                    {subName && <span className="text-[10px] text-zinc-400 bg-zinc-100 px-1.5 py-0.5 rounded ml-2">{subName}</span>}
                                </div>
                                <div className="text-[10px] text-zinc-400 truncate mt-0.5">{item.display_name}</div>
                            </div>
                        )
                    })}
                </div>
...
function ExportModal({ trip, day, activities, onClose, showToast }: any) {
    ...
    const THEMES = {
        modern: { bg: 'bg-zinc-50', paper: 'bg-white', text: 'text-zinc-900', accent: 'bg-indigo-600', accentText: 'text-indigo-600', line: 'bg-indigo-100', dot: 'border-indigo-600' },
        ...
        night: { bg: 'bg-[#0F172A]', paper: 'bg-[#1E293B]', text: 'text-white', accent: 'bg-sky-500', accentText: 'text-sky-400', line: 'bg-slate-700', dot: 'border-sky-500' }
    };
    const t = THEMES[theme];

    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
            <div className="flex flex-col md:flex-row h-[90vh] max-w-5xl w-full bg-zinc-900 rounded-3xl overflow-hidden shadow-2xl">
                <div className="w-full md:w-80 bg-zinc-800 border-r border-zinc-700 flex flex-col">
                    ...
                    <div className="p-6 border-t border-zinc-700 bg-zinc-800">
                        <button onClick={handleDownload} disabled={isGenerating} className="w-full flex items-center justify-center gap-2 bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-500 disabled:opacity-50 transition-all shadow-lg shadow-indigo-900/50">
                ...
                <div className="flex-1 bg-zinc-950 overflow-y-auto p-8 flex justify-center">
                    <div ref={contentRef} className={cn("w-[390px] min-h-[800px] shadow-2xl relative flex flex-col", t.bg)}>
                        <div className="relative h-64 w-full overflow-hidden rounded-b-[40px] shadow-lg z-10">
                            ...
                            <div className="absolute top-6 left-6 text-white/90 text-xs font-bold tracking-[0.2em] uppercase border border-white/30 px-3 py-1 rounded-full backdrop-blur-md">Trip Guide</div>
                        ...
                        <div className="px-6 -mt-6 relative z-20 flex gap-3">
                            <div className={cn("flex-1 p-3 rounded-2xl shadow-lg flex flex-col items-center justify-center gap-1", t.paper)}><div className={cn("text-[10px] font-bold uppercase tracking-wider opacity-60", t.text)}>Budget</div><div className={cn("text-lg font-bold flex items-center gap-0.5", t.text)}><span className="text-xs">¥</span>{totalCost.toLocaleString()}</div></div>
                            <div className={cn("flex-1 p-3 rounded-2xl shadow-lg flex flex-col items-center justify-center gap-1", t.paper)}><div className={cn("text-[10px] font-bold uppercase tracking-wider opacity-60", t.text)}>Spots</div><div className={cn("text-lg font-bold", t.text)}>{spotCount}</div></div>
                            <div className={cn("flex-1 p-3 rounded-2xl shadow-lg flex flex-col items-center justify-center gap-1", t.paper)}><div className={cn("text-[10px] font-bold uppercase tracking-wider opacity-60", t.text)}>Food</div><div className={cn("text-lg font-bold", t.text)}>{foodCount}</div></div>
                        </div>
...
// ... UserMenu, SettingsModal, TABS, MemoriesTab, ChecklistTab, ShareModal, ActivityModal, ConfirmModal (这些代码与之前相同，但需要包含在文件中) ...
// 为了节省篇幅，这里假设你已经包含了上面所有的辅助组件。
// 下面是主组件 PlanDetail 的关键修改部分，确保你复制了 ExportModal 和 LocationSearch。
    ...
    return (
        <div className="relative" ref={menuRef}>
            <button onClick={() => setIsOpen(!isOpen)} className="w-9 h-9 rounded-full bg-zinc-100 border border-zinc-200 overflow-hidden hover:ring-2 hover:ring-zinc-200 transition-all">
                {user.user_metadata?.avatar_url ? (<img src={user.user_metadata.avatar_url} alt="User" className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-600 text-white font-bold text-xs">{user.email?.[0].toUpperCase()}</div>)}
            </button>
            {isOpen && (
                <div className="absolute right-0 top-11 w-56 bg-white rounded-2xl shadow-xl shadow-zinc-200/50 border border-zinc-100 p-2 z-50 animate-in zoom-in-95 origin-top-right">
                    <div className="px-3 py-2 border-b border-zinc-50 mb-1"><p className="text-xs font-bold text-zinc-900 truncate">{user.email}</p><p className="text-[10px] text-zinc-400">Basic Plan</p></div>
                    <button onClick={() => { setIsOpen(false); onOpenSettings(); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-zinc-600 hover:bg-zinc-50 rounded-xl transition-colors"><Settings size={16} /> 设置</button>
                    <button onClick={onLogout} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-xl transition-colors"><LogOut size={16} /> 退出登录</button>
                </div>
            )}
        </div>
    )
    ...
    return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 animate-in zoom-in-95 border border-white/50">
                <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold text-zinc-900">应用设置</h2><button onClick={onClose} className="p-2 hover:bg-zinc-100 rounded-full"><X size={20}/></button></div>
                <div className="space-y-6">
                    <div className="flex items-center justify-between p-4 rounded-2xl bg-zinc-50 border border-zinc-100">
                        <div className="flex items-center gap-3"><div className="p-2 bg-white rounded-xl shadow-sm"><Sun size={18} className="text-amber-500"/></div><div><h3 className="text-sm font-bold text-zinc-900">主题外观</h3><p className="text-xs text-zinc-500">当前：浅色模式</p></div></div>
...
export default function PlanDetail() {
  ...
  const [showFullMap, setShowFullMap] = useState(false); // 🌟 新增：全屏地图状态
  ...
  if (!trip && !loading) return (<div className="flex flex-col h-screen items-center justify-center bg-zinc-50 text-center gap-4"><AlertTriangle size={48} className="text-zinc-300"/><h2 className="text-xl font-bold">无法找到行程</h2><button onClick={() => router.push('/')} className="px-6 py-2 bg-black text-white rounded-full">返回首页</button></div>);
  ...
  return (
    <div className="bg-[#FAFAFA] min-h-screen font-sans text-zinc-900 pb-24 selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <nav className="fixed top-0 inset-x-0 z-40 h-16 bg-white/80 backdrop-blur-xl border-b border-zinc-200/60">
        <div className="max-w-6xl mx-auto px-4 h-full flex items-center justify-between">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 flex items-center p-1 bg-zinc-100/80 rounded-lg border border-zinc-200/50">
                {TABS.map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn("flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-bold transition-all", activeTab === tab.id ? "bg-white text-indigo-600 shadow-sm border border-zinc-200" : "text-zinc-500 hover:text-zinc-900 border border-transparent")}>
            ...
            <div className="flex items-center gap-3">
                ...
                {activeTab === 'timeline' && canEdit && (
                    <button onClick={() => setIsEditing(!isEditing)} className={cn("flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md shadow-sm transition-all border", isEditing ? "text-white bg-indigo-600 border-indigo-600" : "text-zinc-700 bg-white hover:bg-zinc-50 border-zinc-200")}>
                ...
                {activeTab === 'timeline' && !isEditing && (
                    <button onClick={() => setShowExportModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-zinc-900 text-white hover:bg-zinc-800 rounded-md shadow-sm transition-all">
      ...
      <main className="max-w-6xl mx-auto px-4 pt-24">
        ...
        {activeTab === 'timeline' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-10 animate-in fade-in duration-500">
                ...
                <div className="lg:col-span-4 bg-gradient-to-br from-indigo-50 to-white border border-indigo-100/50 rounded-2xl p-6 flex flex-col justify-between h-48 sm:h-64 shadow-sm relative overflow-hidden">
                     <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-full blur-3xl opacity-60 -mr-10 -mt-10 pointer-events-none"></div>
                     <div>
                        ...
                        <div className="flex items-center gap-2 mt-3">
                             ...
                             <span className="text-xs text-zinc-500 font-medium">剩余 ¥{(budgetLimit - totalCost).toLocaleString()}</span>
                        </div>
                     </div>
                     <div className="space-y-3 relative z-10">
                        <div className="flex justify-between text-[10px] font-semibold text-zinc-500">
                            <span>已用 {budgetPercent}%</span>
                            <span className="text-indigo-600 cursor-pointer hover:underline">详情</span>
                        </div>
        ...
        {activeTab === 'timeline' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-7 xl:col-span-8">
                    <div className="sticky top-16 z-30 bg-[#FAFAFA]/95 backdrop-blur-sm py-4 mb-4 flex items-center justify-between border-b border-zinc-200/50">
                         ...
                         <div className="flex gap-2">
                            <button onClick={() => setCurrentDayIdx(c => Math.max(0, c - 1))} disabled={currentDayIdx===0} className="w-8 h-8 rounded-full border border-zinc-200 bg-white flex items-center justify-center hover:border-indigo-300 hover:text-indigo-600 shadow-sm disabled:opacity-30 transition-all"><ChevronLeft size={16}/></button>
                            <button onClick={() => setCurrentDayIdx(c => Math.min(days.length-1, c + 1))} disabled={currentDayIdx===days.length-1} className="w-8 h-8 rounded-full border border-zinc-200 bg-white flex items-center justify-center hover:border-indigo-300 hover:text-indigo-600 shadow-sm disabled:opacity-30 transition-all"><ChevronRight size={16}/></button>
                         </div>
                    </div>
                    ...
                    <div className="relative pl-4 space-y-0 pb-20">
                            ...
                            return (
                                <div key={item.id} className="relative pl-12 pb-8 group/item">
                                    <SortableItem id={item.id} disabled={!canEdit || !isEditing}>
                                        <div className="absolute left-[-18px] top-1.5 px-1.5 py-0.5 bg-white border border-zinc-200 rounded text-[10px] font-bold text-zinc-500 shadow-sm z-20">
                                            {item.time}
                                        </div>
                                        ...
                                        <div 
                                            ...
                                            className={cn(
                                            "relative rounded-xl transition-all duration-300 overflow-hidden group/card bg-white",
                                            isEditing 
                                                ? "border border-dashed border-zinc-300 bg-zinc-50/50 p-4 pl-4 cursor-pointer hover:border-zinc-400" 
                                                : "border border-zinc-200 shadow-sm hover:shadow-md hover:border-indigo-200 p-5 cursor-default"
                                        )}>
                                            {!isEditing && (
                                            ...
                                            <div className={cn("flex flex-col gap-3", isEditing && "opacity-90")}>
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <h3 className={cn("font-bold text-slate-900", isEditing ? "text-sm" : "text-sm sm:text-base")}>{item.title}</h3>
                                                    ...
                                                    {!isEditing && item.cost && item.cost > 0 && (
                                                        <span className="text-[10px] font-semibold px-2 py-1 rounded-md bg-zinc-50 border border-zinc-100 text-zinc-500">
                                                ...
                                                {item.memo && (
                                                    <div className={cn("text-xs text-zinc-600 leading-relaxed flex gap-2", !isEditing && "bg-zinc-50 p-2.5 rounded-lg border border-zinc-100/80")}>
                        ...
                        {isEditing && (
                            <div className="relative pl-12 pt-2 animate-in fade-in">
                                ...
                                <button onClick={() => { setEditingActivity(null); setShowModal(true); }} className="w-full h-12 rounded-xl border border-dashed border-zinc-300 flex items-center justify-center gap-2 text-xs font-semibold text-zinc-500 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50/50 transition-all group">
                                    <span className="w-5 h-5 rounded-full bg-zinc-100 group-hover:bg-indigo-100 text-zinc-400 group-hover:text-indigo-600 flex items-center justify-center transition-colors">
                                        <Plus size={12} />
                                    </span>
                                    添加活动
                                </button>
                ...
                <div className="hidden lg:block lg:col-span-5 xl:col-span-4">
                     <div className="sticky top-24 space-y-6">
                        <div className="bg-white border border-zinc-200 rounded-2xl p-1 shadow-sm h-[340px] relative group hover:shadow-md transition-shadow">
                            ...
                            <div className="absolute bottom-4 left-4 right-4">
                                <button onClick={() => setShowFullMap(true)} className="w-full py-2 bg-white/90 backdrop-blur shadow-sm border border-white/50 rounded-lg text-xs font-semibold text-zinc-700 hover:text-indigo-600 hover:bg-white transition-colors flex items-center justify-center gap-2">
                                    <Maximize2 size={12}/> 打开详细地图
                                </button>
                            </div>
                        </div>

                        <div className="bg-gradient-to-br from-white to-zinc-50 border border-zinc-200 rounded-2xl p-5 shadow-sm relative">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="font-bold text-xs text-indigo-900 uppercase tracking-wider flex items-center gap-2">
                                    <span className="p-1 bg-indigo-100 rounded text-indigo-600"><CheckSquare size={12}/></span>
                                    行前清单
                                </h3>
                                <button onClick={() => setActiveTab('checklist')} className="text-[10px] text-indigo-600 font-medium hover:underline">查看全部</button>
                            </div>
                            ...
                            {isEditing && (
                                <button onClick={() => setActiveTab('checklist')} className="mt-4 w-full py-2 text-[10px] font-medium text-zinc-400 hover:text-indigo-600 border-t border-zinc-200/50 transition-colors flex items-center justify-center gap-1">
                                    <Plus size={10} /> 管理清单
                                </button>
                            )}
                        </div>
        ...
        {activeTab === 'checklist' && <ChecklistTab categories={checklistCategories} onToggle={toggleChecklistItem} />}
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-zinc-100 mb-8">
                <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                        <textarea value={newPost} onChange={e => setNewPost(e.target.value)} placeholder="分享旅途中的精彩瞬间..." className="w-full bg-transparent border-none focus:ring-0 resize-none text-zinc-800 placeholder:text-zinc-400 text-sm h-20 p-0" />
                ...
                {posts.map(post => (
                    <div key={post.id} className="bg-white rounded-3xl p-6 shadow-sm border border-zinc-100">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <div><h4 className="font-bold text-zinc-900 text-sm">{post.user}</h4><p className="text-[10px] text-zinc-400">{post.time}</p></div>
                        </div>
                        ...
                        {post.images.length > 0 && (
                            <div className={cn("grid gap-2 mb-4 rounded-xl overflow-hidden", post.images.length === 1 ? "grid-cols-1" : "grid-cols-2")}>
                                {post.images.map((img, i) => (<img key={i} src={img} className="w-full h-48 object-cover hover:scale-105 transition-transform duration-500 cursor-zoom-in" />))}
                            </div>
                        )}
                        <div className="flex items-center gap-6 border-t border-zinc-50 pt-3">
                            <button onClick={() => toggleLike(post.id)} className={cn("flex items-center gap-1.5 text-xs font-bold transition-colors", post.liked ? "text-red-500" : "text-zinc-400 hover:text-zinc-600")}><Heart size={16} fill={post.liked ? "currentColor" : "none"} /> {post.likes}</button>
                            <button className="flex items-center gap-1.5 text-xs font-bold text-zinc-400 hover:text-zinc-600 transition-colors"><MessageCircle size={16} /> 评论</button>
                        </div>
                    </div>
                ))}
            </div>
...
function ChecklistTab({ categories, onToggle }: { categories: ChecklistCategory[], onToggle: (cid: string, iid: number) => void }) {
                ...
                {categories.map(cat => (
                    <div key={cat.id} className="bg-white border border-zinc-200 rounded-3xl p-5 shadow-sm">
                        ...
                        <div className="space-y-2">
                            {cat.items.map(item => (
                                <div key={item.id} onClick={() => onToggle(cat.id, item.id)} className="flex items-center gap-3 p-2 hover:bg-zinc-50 rounded-xl cursor-pointer transition-colors group">
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
             ...
             <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 animate-in zoom-in-95 border border-white/50">
                 ...
                 <div className="bg-zinc-50 p-4 rounded-2xl flex items-center gap-3 mb-6"><div className="flex-1 truncate text-xs text-zinc-500 font-mono">{window.location.href}</div><button onClick={handleCopy} className="text-xs font-bold bg-white border border-zinc-200 px-3 py-1.5 rounded-lg shadow-sm hover:bg-zinc-50">{copied ? '已复制' : '复制链接'}</button></div>
                 <div className="space-y-4"><h4 className="text-xs font-bold text-zinc-500 uppercase">邀请协作者</h4><div className="flex gap-2"><input type="email" placeholder="输入邮箱地址" className="flex-1 px-4 py-2 bg-white border border-zinc-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500/20" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} /><button onClick={handleInvite} className="bg-zinc-900 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-zinc-800">邀请</button></div></div>
             </div>
        </div>
    )
}
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
             ...
             <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl p-6 animate-in zoom-in-95">
                 <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold text-zinc-900">{initialData ? '编辑活动' : '添加新活动'}</h3><button onClick={onClose}><X size={20}/></button></div>
                 <div className="space-y-4">
                    <div className="grid grid-cols-4 gap-2">{Object.entries(TYPE_CONFIG).map(([key, cfg]) => (<button key={key} onClick={() => setFormData({...formData, type: key})} className={cn("flex flex-col items-center gap-1 p-2 rounded-xl border transition-all", formData.type === key ? `bg-zinc-900 text-white border-zinc-900` : "bg-white border-zinc-200 text-zinc-500 hover:bg-zinc-50")}><cfg.icon size={16}/><span className="text-[10px] font-medium">{cfg.label}</span></button>))}</div>
                    <div className="grid grid-cols-3 gap-4"><div className="col-span-1"><label className="text-xs font-bold text-zinc-500 mb-1 block">时间</label><input type="time" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} className="w-full p-3 bg-zinc-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-zinc-200 font-mono text-sm"/></div><div className="col-span-2"><label className="text-xs font-bold text-zinc-500 mb-1 block">活动名称</label><input type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="例如：春熙路" className="w-full p-3 bg-zinc-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-zinc-200 text-sm font-medium"/></div></div>
                    ...
                    <div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-zinc-500 mb-1 block">预算 (CNY)</label><input type="number" value={formData.cost} onChange={e => setFormData({...formData, cost: Number(e.target.value)})} className="w-full p-3 bg-zinc-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-zinc-200 text-sm"/></div></div>
                    <div><label className="text-xs font-bold text-zinc-500 mb-1 block">备注</label><textarea value={formData.memo} onChange={e => setFormData({...formData, memo: e.target.value})} rows={3} placeholder="添加备注信息..." className="w-full p-3 bg-zinc-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-zinc-200 text-sm resize-none"/></div>
                 </div>
                 <button onClick={handleSubmit} className="w-full py-3 bg-zinc-900 text-white rounded-xl font-bold mt-6 hover:bg-zinc-800 transition-colors">保存</button>
             </div>
        </div>
    )
}
